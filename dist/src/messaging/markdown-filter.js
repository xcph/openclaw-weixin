...
export class StreamingMarkdownFilter {
    ...
    pump(eof) {
        ...
        if (eof && this.inl) {
            if (this.inl.type === "table") {
                ...
                const markers = { code: "`", image: "![", strike: "~~", bold3: "***", italic: "*", ubold3: "___", uitalic: "_" };
    ...
    static extractTableRow(line) {
        if (/^[\s|:\-]+$/.test(line) && line.includes("-"))
            return "";
        const parts = line.split("|").map(c => c.trim());
        const cells = parts.slice(parts[0] === "" ? 1 : 0, parts[parts.length - 1] === "" ? parts.length - 1 : parts.length);
        return cells.join("\t");
    }
}
