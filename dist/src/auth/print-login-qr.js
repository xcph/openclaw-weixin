...
function createEmitQr(log, writeStdout) {
    return (block) => {
...
function coerceQrcodeTerminal(mod) {
    if (!mod || typeof mod !== "object") {
        return null;
    }
    const root = mod;
    const inner = root.default && typeof root.default === "object" ? root.default : root;
    const gen = inner.generate;
    return typeof gen === "function" ? inner : null;
}
/**
 ...
 */
function resolveOpenClawHostPackageJson() {
 ...
 */
export async function printWeixinLoginQrToConsole(qrcodeUrl, log, writeStdout) {
    log(`扫码链接（可复制到浏览器打开）：\n${qrcodeUrl}`);
    ...
    if (QRCode) {
        ...
        for (const opts of rasterAttempts) {
            try {
                ...
                logger.warn(`printWeixinLoginQrToConsole: qrcode ${opts.type}${small === undefined ? "" : ` small=${small}`} err=${String(err)}`);
            }
        }
    }
    if (!rendered) {
        try {
            ...
            if (termSync) {
                await new Promise((resolve, reject) => {
                    ...
                    try {
                        termSync.generate(qrcodeUrl, { small: false }, (qr) => {
                            clearTimeout(t);
                ...
                await new Promise((resolve, reject) => {
                    const t = setTimeout(() => reject(new Error("qrcode-terminal timed out")), QRCODE_TERMINAL_CB_MS);
                    try {
                        qrcodeterminal.default.generate(qrcodeUrl, { small: false }, (qr) => {
                            clearTimeout(t);
                            if (typeof qr === "string" && qr.trim()) {
                                emitQr(qr);
                                rendered = true;
        ...
        try {
            ...
            if (termSync) {
                await new Promise((resolve, reject) => {
                    const t = setTimeout(() => reject(new Error("qrcode-terminal timed out")), QRCODE_TERMINAL_CB_MS);
                    try {
                        termSync.generate(qrcodeUrl, { small: true }, (qr) => {
                            clearTimeout(t);
                            if (typeof qr === "string" && qr.trim()) {
                                emitQr(qr);
                                rendered = true;
                ...
                await new Promise((resolve, reject) => {
                    const t = setTimeout(() => reject(new Error("qrcode-terminal timed out")), QRCODE_TERMINAL_CB_MS);
                    try {
                        qrcodeterminal.default.generate(qrcodeUrl, { small: true }, (qr) => {
                            clearTimeout(t);
                            if (typeof qr === "string" && qr.trim()) {
                                emitQr(qr);
                                rendered = true;
...
//# sourceMappingURL=print-login-qr.js.map