...
export const weixinPlugin = {
    ...
    configSchema: {
        schema: {
            ...
            properties: {
                showThinking: {
                    type: "boolean",
                    description: "When true, forward model thinking/reasoning stream to Weixin as plain-text messages (💭 prefix). Per-account `accounts.<id>.showThinking` overrides. If omitted, falls back to legacy `showReasoning`.",
                },
                showReasoning: {
                    type: "boolean",
                    description: "Deprecated alias for `showThinking`. When true, forward model reasoning/thinking to Weixin as plain-text messages (💭 prefix) and log previews on the gateway. Per-account `accounts.<id>.showReasoning` overrides section default only when `showThinking` is unset.",
                },
                showTools: {
                    type: "boolean",
    ...
    auth: {
        login: (async (params) => {
            ...
            const writeStdout = runtime &&
                typeof runtime === "object" &&
                "writeStdout" in runtime &&
                typeof runtime.writeStdout === "function"
                ? runtime.writeStdout.bind(runtime)
                : undefined;
            ...
            if (waitResult.connected && waitResult.botToken && waitResult.accountId) {
                try {
                    ...
                    await triggerWeixinChannelReload().catch((err) => logger.warn(`triggerWeixinChannelReload: ${String(err)}`));
                    log(`\n✅ 与微信连接成功！`);
                    log(`凭证已写入状态目录；网关若已在运行，长轮询会在下一轮自动读取新 token（通常无需重启）。`);
