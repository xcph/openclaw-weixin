...
const chatLoginInFlight = new Map<string, Promise<void>>();

/** @internal 单元测试之间清空并发状态 */
export function resetWeixinChatLoginStateForTest(): void {
  chatLoginInFlight.clear();
}

/** `renew`：当前 accountId 会话续登；`bind-new`：新增一条账号（不传 accountId 给 QR 起点） */
export type WeixinChatLoginIntent = "renew" | "bind-new";

/** 解析 `/weixin-login` 后的参数，例如 `new`、`bind` */
export function parseWeixinLoginIntent(args: string): WeixinChatLoginIntent {
...
/** 与 SlashCommandContext 字段对齐（避免与 slash-commands 循环依赖） */
export type WeixinLoginSlashContext = {
  to: string;
  contextToken?: string;
  baseUrl: string;
  token?: string;
...
function resolveEffectiveAllowFrom(accountId: string): string[] {
...
function effectiveApiBase(accountId: string): string {
  return loadWeixinAccount(accountId)?.baseUrl?.trim() || DEFAULT_BASE_URL;
}

async function sendReply(ctx: WeixinLoginSlashContext, text: string): Promise<void> {
...
async function persistLoginSuccess(result: WeixinQrWaitResult): Promise<void> {
...
async function runBackgroundWeixinLogin(
  ctx: WeixinLoginSlashContext,
  apiBaseUrl: string,
  intent: WeixinChatLoginIntent,
): Promise<void> {
  try {
 ...
 */
export async function scheduleWeixinChatLogin(
  ctx: WeixinLoginSlashContext,
  intent: WeixinChatLoginIntent = "renew",
): Promise<void> {
  ...
  if (allowed.length === 0) {
    await sendReply(
      ctx,
      "当前账号尚未配置配对用户（allowFrom），无法校验权限。请先在 OpenClaw 中完成配对后再使用 /weixin-login。",
    );
    return;
  }
