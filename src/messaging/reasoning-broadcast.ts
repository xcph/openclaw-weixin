...
export type ReasoningBroadcastOptions = {
  /** Serialize sends so ordering matches stream and API limits are respected. */
  sendText: (text: string) => Promise<void>;
  log: ReasoningLogger;
  /** Debounce window before flushing a partial chunk (ms). */
  debounceMs?: number;
  /** Minimum buffered characters before a non-final flush. */
  minFlushChars?: number;
  /** Max characters per Weixin message body (leave margin below transport limits). */
...
export function createWeixinReasoningBroadcast(opts: ReasoningBroadcastOptions): {
  ...
  const enqueueSend = (fn: () => Promise<void>) => {
    sendChain = sendChain.then(fn).catch((err: unknown) => {
  ...
  const flushInternal = async (mode: "stream-tick" | "debounced-tick" | "finalize") => {
    ...
    while (true) {
      ...
      if (message) {
        opts.log.info(
          `reasoning outbound: len=${body.length} preview=${previewForLog(body)}`,
        );
  ...
  const flush = async (mode: "stream-tick" | "debounced-tick" | "finalize") => {
...
function previewForLog(text: string, max = 160): string {
