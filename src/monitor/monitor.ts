...
export async function monitorWeixinProvider(opts: MonitorWeixinOpts): Promise<void> {
  ...
  const recordSeenMessageKey = (key: string, now: number): void => {
  ...
  const pruneSeenMessageKeys = (now: number): void => {
    ...
    try {
      ...
      for (const full of list) {
        ...
        if (seenKey !== undefined && seenMessageKeys.has(seenKey)) {
          aLog.warn(
            `skipping duplicate inbound message (key=${seenKey}) — likely -14 cursor replay`,
          );
          continue;
        }
