...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  meta: {
    ...
    blurb:
      "getUpdates long-poll; send text or media (image / video / file attachment) downstream; token auth.",
    order: 75,
  },
  configSchema: {
    schema: {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "Non-image attachments are routed as WeChat file messages from extension / Content-Type; use a path or URL whose filename exposes the type when possible.",
