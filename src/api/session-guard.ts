...
function resolveWeixinSessionPausePath(accountId: string): string {
...
function linkedPauseAccountIds(accountId: string): string[] {
  const trimmed = accountId.trim();
  const norm = normalizeAccountId(trimmed);
  const ids = new Set<string>([trimmed, norm]);
...
function readPauseUntilFromDisk(accountId: string): number | undefined {
  ...
  try {
    ...
    const until = (parsed as { until: number }).until;
...
function writePauseUntilToDisk(accountId: string, until: number): void {
  const filePath = resolveWeixinSessionPausePath(accountId);
...
function unlinkPauseFile(accountId: string): boolean {
  const filePath = resolveWeixinSessionPausePath(accountId);
  try {
...
export function pauseSession(accountId: string): void {
  ...
  logger.info(
    `session-guard: paused accountId=${canonical} until=${new Date(until).toISOString()} (${SESSION_PAUSE_DURATION_MS / 1000}s, persisted)`,
  );
}
...
export function getRemainingPauseMs(accountId: string): number {
  ...
  if (maxUntil === undefined) return 0;
 ...
 * @internal
 */
export function _resetForTest(accountIds: string[] = ["acc1", "acc2", "ghost"]): void {
