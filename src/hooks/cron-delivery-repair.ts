...
function hasConcreteTo(to: unknown): boolean {
...
function repairDelivery(
  ...
  if (mode !== "announce") return null; // 非 announce（如 silent）不接管
  const channel = base.channel;
  // 指向别的渠道 → 不碰；缺失 或 本渠道 → 接管
  if (typeof channel === "string" && channel !== "" && channel !== CHANNEL_ID) return null;
  const needChannel = channel !== CHANNEL_ID;
  const needTo = !hasConcreteTo(base.to);
  if (!needChannel && !needTo) return null; // 已完整
  const next: Record<string, unknown> = { ...base, mode, channel: CHANNEL_ID };
  if (needTo) next.to = to;
  if (accountId && !base.accountId) next.accountId = accountId;
  return next;
}
 ...
 * 无论模型是否正确填写。
 */
export function registerCronDeliveryRepairHook(api: OpenClawPluginApi): void {
  if (typeof api.on !== "function") return;
  api.on("before_tool_call", (event) => {
    if (event.toolName !== "cron") return;
    const params = event.params;
    if (!params || typeof params !== "object" || params.action !== "add") return;
    const job = params.job;
    if (!job || typeof job !== "object") return;
    const jobObj = job as Record<string, unknown>;

    const to = getRequestTarget();
    if (!to) return; // 不在本渠道会话上下文中，跳过
