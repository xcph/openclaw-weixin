...
function repairDelivery(
  ...
  if (delivery.channel !== CHANNEL_ID) return null;
  if (hasConcreteTo(delivery.to)) return null;
  const next: Record<string, unknown> = { ...delivery, to };
  if (accountId && !delivery.accountId) next.accountId = accountId;
  return next;
}
...
export function registerCronDeliveryRepairHook(api: OpenClawPluginApi): void {
  ...
  api.on("before_tool_call", (event) => {
    ...
    const repaired = repairDelivery(delivery as Record<string, unknown>, to, accountId);
    if (!repaired) return;
